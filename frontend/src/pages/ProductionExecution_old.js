      ...
      if (stagesResponse.data.data && stagesResponse.data.data.length > 0) {
        ...
        for (const plan of stagesResponse.data.data) {
          try {
            ...
            const planStages = stagesResponse.data.data || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updateData = {
        ...
        completedBy: formData.status === 'completed' ? new Date() : null
      };
...
const DowntimeModal = ({ stage, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Get current downtime logs and add new one
      const currentStage = stage;
      const updatedLogs = [...(currentStage.downtimeLog || []), downtime];

      await productionAPI.updateStage(stage._id, {
        downtimeLog: updatedLogs
      });
